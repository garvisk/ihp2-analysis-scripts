...
def generate_two_vehicle_speed_plot(bag_file_name, leader_vehicle_rosbag, follower_vehicle_rosbag, time_starting_vehicle_start_engagement, time_starting_vehicle_end_engagement, veh_1_speed_limit_zone_change_times, first_joiner_platoon_start_time, test_type, test_num, joiner_vehicle_num):
    ...
    ax.set_title(str(test_type) + " Test Case Run " + str(test_num) + " Vehicle Speeds -- Vehicle " + str(joiner_vehicle_num) + " ( " + str(bag_file_name) + ")") # Plot Title
...
def generate_three_vehicle_speed_plot(bag_file_name, vehicle_1_rosbag, vehicle_2_rosbag, vehicle_3_rosbag, time_starting_vehicle_start_engagement, time_starting_vehicle_end_engagement, first_joiner_platoon_start_time, second_joiner_platoon_start_time, test_type, test_num):
    # Create the initial plot with the defined figure size
    fig, ax = plt.subplots(figsize=(9,5.5))
    ...
    if test_type == "SLR" or test_type == "ALR":
        ...
        # Plot vertical bar at time that first follower joins platoon
        ax.axvline(x = (first_joiner_platoon_start_time - time_start).to_sec(), color = 'b', linestyle='-', label = 'Time Vehicle 2 Joins Platoon')

        # Plot vertical bar at time that second follower joins platoon
        ax.axvline(x = (second_joiner_platoon_start_time - time_start).to_sec(), color = 'r', linestyle=':', label = 'Time Vehicle 3 Joins Platoon',  linewidth=3)
        ...
        # Plot vertical bar at time that first follower joins platoon
        ax.axvline(x = (first_joiner_platoon_start_time - time_start).to_sec(), color = 'r', linestyle='-', label = 'Time Vehicle 3 Joins Platoon')

        # Plot vertical bar at time that second follower joins platoon
        ax.axvline(x = (second_joiner_platoon_start_time - time_start).to_sec(), color = 'g', linestyle=':', label = 'Time Vehicle 1 Joins Platoon',  linewidth=3)

    # Plot vehicle 1 speed (m/s) vs. time
    ax.plot(vehicle_1_speed_times, vehicle_1_speeds, 'g', linestyle='-.', label='Vehicle 1 Speed (m/s)')

    # Plot vehicle 2 speed (m/s) vs. time
    ax.plot(vehicle_2_speed_times, vehicle_2_speeds, 'b', linestyle=':', label='Vehicle 2 Speed (m/s)')

    # Plot vehicle 3 speed (m/s) vs. time
    ax.plot(vehicle_3_speed_times, vehicle_3_speeds, 'r', linestyle='--', label='Vehicle 3 Speed (m/s)')

    plt.rc('axes', labelsize=12)  # fontsize of the axes labels
    plt.rc('legend', fontsize=10)  # fontsize of the legend text
    plt.grid()
    ax.legend(loc = 'lower right') # Location of the legend
    ax.set_title(str(test_type) + ": All Vehicle Speeds" + " (Leader: " + str(bag_file_name) + ")") # Plot Title
    ax.set_xlabel("Time (seconds) Since Start of Engagement for Starting Vehicle") # Plot X Title
    ax.set_ylabel("Vehicle Speed (m/s)") # Plot Y Title
    ax.set_xlim([-5, 175])
    ax.set_ylim([0, 20])
...
def generate_platooning_plot(bag, time_start_platooning, bag_file_name, test_type, test_num, vehicle_number, first_start_time, platooning_start_time):
    # num = 0
    ...
    for topic, msg, t in bag.read_messages(topics=['/guidance/platooning_info'], start_time = time_start_platooning):
        ...
        times.append((t-first_start_time).to_sec())
    ...
    # Plot actual gaps (meters) vs. time Jon here
    # first_start_time = first_start_time.to_sec() 
    # platooning_start_time = platooning_start_time.to_sec()
    # new_diff = (platooning_start_time - first_start_time) #New offset for start time
    # print("new diff is")
    # print(new_diff)
    # new_time = [x + new_diff for x in times]

    # ax.plot(new_time, actual_gaps, 'g:', label='Actual Gap (meters)')
    ...
    plt.grid()
    ax.legend(loc = 'lower right') # Location of the legend
    ax.set_title(str(test_type) + ": Platooning Information - Gap -- Vehicle " + str(vehicle_number) + " (" + str(bag_file_name) + ")") # Plot Title
    ax.set_xlabel("Time (seconds) Since First Vehicle Started") # Plot X Title
    ax.set_ylabel("Gap (Meters)") # Plot Y Title
    ax.set_xlim([-5, 175])
    ax.set_ylim([0, 60])
...
def received_tcm_analysis(bag, time_start_platooning, test_type, vehicle_number, expected_tcm_count):
    ...
    tcm_receive_times = [-9999 for i in range(0, expected_tcm_count)]
    ...
    for topic, msg, t in bag.read_messages(topics=['/message/incoming_geofence_control']): 
        ...
        if reqid in tcr_reqids:
            ...
            if tcm_receive_times[msgnum] is None:
                ...
                print("DEBUG:Kevin - tcm_type is " + str(tcm_type))
                if tcm_type == 12:
    ...
    if not has_received_any_tcms:
        ...
        return
    print("Kevin - Has recieved all tcms? " + str(has_received_all_tcms))
...
def downtrack_distance_analysis(join_vehicle_rosbag, other_vehicle_rosbag, run_type, join_vehicle_number, other_vehicle_number, time):
    ...
    for topic, msg, t in join_vehicle_rosbag.read_messages(topics=['/hardware_interface/vehicle/twist'], start_time = time):
        ...
        break
    print(other_vehicle_pose)
    print(join_vehicle_pose)
    print(join_vehicle_speed)
...
def main():  
    ...
    slr_bag_files = [[["WP_MPH_1_2022-07-20-14-22-06.bag",
                           "BP_MPH_1_2022-07-20-14-21-27.bag",
                           "BL_MPH_1_2022-07-20-14-21-31.bag"], 
                           [WHITE_PACIFICA, BLACK_PACIFICA, BLUE_LEXUS]],
                     [["WP_MPH_2_2022-07-20-14-34-58.bag",
                           "BP_MPH_2_2022-07-20-14-33-24.bag",
                           "BL_MPH_2_2022-07-20-14-33-54.bag"], 
                           [WHITE_PACIFICA, BLACK_PACIFICA, BLUE_LEXUS]],
                     [["WP_MPH_3_2022-07-20-14-43-30.bag",
                           "BP_MPH_3_2022-07-20-14-42-51.bag",
                           "BL_MPH_3_2022-07-20-14-41-37.bag"], 
                           [WHITE_PACIFICA, BLACK_PACIFICA, BLUE_LEXUS]],
                     [["WP_MPH_4_2022-07-20-14-49-15.bag",
                           "BP_MPH_4_2022-07-20-14-49-02.bag",
                           "BL_MPH_4_2022-07-20-14-49-50.bag"], 
                           [WHITE_PACIFICA, BLACK_PACIFICA, BLUE_LEXUS]],
                    [["WP_MPH_5_2022-07-20-14-55-30.bag",
                           "BP_MPH_5_2022-07-20-14-55-13.bag",
                           "BL_MPH_5_2022-07-20-14-57-07.bag"], 
                           [WHITE_PACIFICA, BLACK_PACIFICA, BLUE_LEXUS]]]
    ...
    for test_case_files in ihp2_bag_files:
        ...
        for file_number in range(0, len(bag_files_list)):
            ...
            if vehicle_number == 2:
                ...
                # second_start_diff = time_2_start_engagement - time_1_start_engagement
            ...
            if vehicle_number == 3:
                ...
                # third_start_diff = time_3_start_engagement - time_1_start_engagement
        ...
        if test_type == "SLR":
            ...
            generate_two_vehicle_speed_plot(vehicle_2_rosbag_name, vehicle_1_rosbag, vehicle_2_rosbag, time_1_start_engagement, time_1_end_engagement, veh_1_speed_limit_zone_change_times, vehicle_2_platooning_times[1], test_type, test_num, joiner_vehicle_num=2)
            generate_two_vehicle_speed_plot(vehicle_3_rosbag_name, vehicle_1_rosbag, vehicle_3_rosbag, time_1_start_engagement, time_1_end_engagement, veh_1_speed_limit_zone_change_times, vehicle_3_platooning_times[1], test_type, test_num, joiner_vehicle_num=3)
            ...
            generate_platooning_plot(vehicle_2_rosbag, vehicle_2_platooning_times[1], vehicle_2_rosbag_name, test_type, test_num, 2, time_1_start_engagement, vehicle_2_platooning_times[1]) #Jon here - SLR seems fine
            #generate_platooning_plot(vehicle_3_rosbag, vehicle_3_platooning_times[1], vehicle_3_rosbag_name, test_type, test_num, 3)
            generate_platooning_plot(vehicle_3_rosbag, vehicle_3_platooning_times[1], vehicle_3_rosbag_name, test_type, test_num, 3, time_1_start_engagement, vehicle_3_platooning_times[1]) #Jon here - SLR seems fine
            
            #generate_three_vehicle_speed_plot(vehicle_1_rosbag_name, vehicle_1_rosbag, vehicle_2_rosbag, vehicle_3_rosbag, time_1_start_engagement, time_1_end_engagement, vehicle_2_platooning_times[1], vehicle_3_platooning_times[1], test_type, test_num)
            ...
            generate_two_vehicle_speed_plot(vehicle_3_rosbag_name, vehicle_2_rosbag, vehicle_3_rosbag, time_2_start_engagement, time_2_end_engagement, veh_1_speed_limit_zone_change_times, vehicle_3_platooning_times[1], test_type, test_num, joiner_vehicle_num=3)
            generate_two_vehicle_speed_plot(vehicle_1_rosbag_name, vehicle_1_rosbag, vehicle_2_rosbag, time_2_start_engagement, time_2_end_engagement, veh_1_speed_limit_zone_change_times, vehicle_1_platooning_times[1], test_type, test_num, joiner_vehicle_num=2)

            downtrack_distance_analysis(vehicle_1_rosbag, vehicle_2_rosbag, test_type, 1, 2, vehicle_1_platooning_times[0])
            downtrack_distance_analysis(vehicle_3_rosbag, vehicle_2_rosbag, test_type, 3, 2, vehicle_3_platooning_times[0])

            generate_platooning_plot(vehicle_2_rosbag, vehicle_1_platooning_times[1], vehicle_2_rosbag_name, test_type, test_num, 2, time_2_start_engagement, vehicle_1_platooning_times[1]) #Jon here - SLF isnt working
            generate_platooning_plot(vehicle_3_rosbag, vehicle_3_platooning_times[1], vehicle_3_rosbag_name, test_type, test_num, 3, time_2_start_engagement, vehicle_3_platooning_times[1]) #Jon here - SLF isnt working

            generate_three_vehicle_speed_plot(vehicle_1_rosbag_name, vehicle_1_rosbag, vehicle_2_rosbag, vehicle_3_rosbag, time_2_start_engagement, time_2_end_engagement, vehicle_3_platooning_times[1], vehicle_1_platooning_times[1], test_type, test_num)
            ...
            generate_two_vehicle_speed_plot(vehicle_2_rosbag_name, vehicle_1_rosbag, vehicle_2_rosbag, time_1_start_engagement, time_1_end_engagement, veh_1_speed_limit_zone_change_times, vehicle_2_platooning_times[1], test_type, test_num, joiner_vehicle_num=2)
            generate_two_vehicle_speed_plot(vehicle_3_rosbag_name, vehicle_1_rosbag, vehicle_3_rosbag, time_1_start_engagement, time_1_end_engagement, veh_1_speed_limit_zone_change_times, vehicle_3_platooning_times[2], test_type, test_num, joiner_vehicle_num=3)
            
            generate_platooning_plot(vehicle_2_rosbag, vehicle_2_platooning_times[1], vehicle_2_rosbag_name, test_type, test_num, 2, time_1_start_engagement, vehicle_2_platooning_times[1]) #Jon here - ALR some seem fine, others are slightly off
            # generate_platooning_plot(vehicle_3_rosbag, vehicle_3_platooning_times[1], vehicle_3_rosbag_name, test_type, test_num, 3)
            generate_platooning_plot(vehicle_3_rosbag, vehicle_3_platooning_times[1], vehicle_3_rosbag_name, test_type, test_num, 3, time_1_start_engagement, vehicle_3_platooning_times[2]) #Jon here - ALR some seem fine, others are slightly off
            
            generate_three_vehicle_speed_plot(vehicle_1_rosbag_name, vehicle_1_rosbag, vehicle_2_rosbag, vehicle_3_rosbag, time_1_start_engagement, time_1_end_engagement, vehicle_2_platooning_times[1], vehicle_3_platooning_times[2], test_type, test_num)
            ...
            generate_two_vehicle_speed_plot(vehicle_3_rosbag_name, vehicle_2_rosbag, vehicle_3_rosbag, time_2_start_engagement, time_2_end_engagement, veh_1_speed_limit_zone_change_times, vehicle_3_platooning_times[1], test_type, test_num, joiner_vehicle_num=3)
            generate_two_vehicle_speed_plot(vehicle_1_rosbag_name, vehicle_1_rosbag, vehicle_2_rosbag, time_2_start_engagement, time_2_end_engagement, veh_1_speed_limit_zone_change_times, vehicle_1_platooning_times[2], test_type, test_num, joiner_vehicle_num=2)

            generate_platooning_plot(vehicle_2_rosbag, vehicle_2_platooning_times[1], vehicle_2_rosbag_name, test_type, test_num, 2, time_2_start_engagement, vehicle_3_platooning_times[1]) #Jon here - ALRF seems pretty broken
            # generate_platooning_plot(vehicle_3_rosbag, vehicle_3_platooning_times[1], vehicle_3_rosbag_name, test_type, test_num, 3)
            generate_platooning_plot(vehicle_3_rosbag, vehicle_3_platooning_times[1], vehicle_3_rosbag_name, test_type, test_num, 3, time_2_start_engagement, vehicle_1_platooning_times[2]) #Jon here - ALRF seems pretty broken
            
            generate_three_vehicle_speed_plot(vehicle_1_rosbag_name, vehicle_1_rosbag, vehicle_2_rosbag, vehicle_3_rosbag, time_2_start_engagement, time_2_end_engagement, vehicle_3_platooning_times[1], vehicle_1_platooning_times[2], test_type, test_num)
            ...
            generate_platooning_plot(vehicle_2_rosbag, vehicle_2_platooning_times[1], vehicle_2_rosbag_name, test_type, test_num, 2, time_1_start_engagement, vehicle_2_platooning_times[1])
            generate_two_vehicle_speed_plot(vehicle_2_rosbag_name, vehicle_1_rosbag, vehicle_2_rosbag, time_1_start_engagement, time_1_end_engagement, veh_1_speed_limit_zone_change_times, vehicle_2_platooning_times[1], test_type, test_num, joiner_vehicle_num=2)
